    ...
    <div class="main__buttons">
      <div class="work__buttons">
        <form class="work__buttons-form" action="/work/start" method="POST">
          @csrf
          <button class="work__button-item" type="submit" {{ $isWorking ? 'disabled' : '' }}>勤務開始</button>
        </form>
        <form class="work__buttons-form" action="/work/end" method="POST">
          @csrf
          <button class="work__button-item" type="submit" {{ !$isWorking ? 'disabled' : ''}}>勤務終了</button>
        </form>
      </div>
      <div class="rest__buttons">
        <form class="rest__buttons-form" action="/rest/start" method="POST" {{ $isWorking ? 'disabled' : '' }}>
          @csrf
